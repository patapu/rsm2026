 ...
 * Uses the same dark theme as the resume.
 */
export const markdownComponents: Components = {
  ...
  h1: ({ children }) => (
    <h1 className="text-lg font-bold mb-2 text-text">{children}</h1>
  ),
  h2: ({ children }) => (
    <h2 className="text-base font-bold mb-2 text-text">{children}</h2>
  ),
  h3: ({ children }) => (
    <h3 className="text-sm font-bold mb-1 text-text">{children}</h3>
  ),
  ul: ({ children }) => (
  ...
  code: ({ children, className }) => {
    ...
    if (isBlock) {
      return (
        <code className="block bg-bg rounded p-2 text-xs font-mono overflow-x-auto">
          {children}
        </code>
      )
    }
    return (
      <code className="bg-bg rounded px-1 py-0.5 text-xs font-mono text-accent">
        {children}
      </code>
    )
  },
